...
def read_camera(camera_queue, sync_event):
    ...
    while True:
        ...
        if ret:
            ...
            sync_event.set()  # 通知其他进程可以处理该帧
            sync_event.clear()  # 重置事件


def detect_lines(camera_queue, line_queue, sync_event):
    processor = ImageProcessor()
    while True:
        sync_event.wait()  # 等待同步信号
        frame = camera_queue.get()
        processed_frame = processor.to2(frame)
        if abs(processor.checkDx(processor.split_image_and_find_centers(processed_frame))) >= timeToLine:
            # 复制一份图像用于绘制直线
            line_frame = frame.copy()
            angles = processor.parallel(line_frame)
            line_queue.put(angles)
            # 显示绘制了直线的图像
            cv2.imshow("Line Detection", line_frame)
            cv2.waitKey(1)
        else:
            line_queue.put((0, 0, 0, 0))


def detect_centers(camera_queue, center_queue, sync_event):
    processor = ImageProcessor()
    while True:
        sync_event.wait()  # 等待同步信号
        frame = camera_queue.get()
        ...
        center_queue.put((centers, error))
...
if __name__ == "__main__":
    ...
    camera_process = multiprocessing.Process(target=read_camera, args=(camera_queue, sync_event))
    line_process = multiprocessing.Process(target=detect_lines, args=(camera_queue, line_queue, sync_event))
    center_process = multiprocessing.Process(target=detect_centers, args=(camera_queue, center_queue, sync_event))
    ...
    while True:
        try:
            ...
            centers, error = center_queue.get()
            angel_int1, angel_float1, angel_int2, angel_float2 = line_queue.get()
